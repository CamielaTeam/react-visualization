var ReactLexer = require("./gramatica/.antlr/ReactLexer").ReactLexer;
var ReactParser = require("./gramatica/.antlr/ReactParser").ReactParser;
var ReactListener = require("./gramatica/.antlr/ReactListener").ReactListener;
...
var index_of_component = null;
var repeatedComponent = null;
var currentPropType = null;
var currentComponentCalls = null;
...
KeyPrinter.prototype.enterHtml_elements = function (ctx) {
  if (
    listOfComponentsInFile.filter((component) => {
      return component.name === currentComponent;
    }).length === 0
  ) {
    listOfComponentsInFile.push({
  ...
  if (listOfImports.includes(ctx.ID()[0].toString())) {
    ...
    if (
      arrayOfComponentCalls.filter((component) => {
    ...
    listOfComponentsInFile[index_of_component].componentsInside[
      ctx.ID()[0].toString()
    ] = listOfComponentsInFile[index_of_component].componentsInside[
      ctx.ID()[0].toString()
    ]
...
KeyPrinter.prototype.enterProps = function (ctx) {
  if (listOfImports.includes(currentComponentCall)) {
    ...
    var possibleRepeatedComponent = arrayOfComponentCalls.filter(
      (component) => {
        return component.name === currentComponentCall;
      }
    )[0];
    ...
    if (!repeated) {
      listOfComponentsInFile[index_of_component].componentsInside[
        ...
        ? listOfComponentsInFile[index_of_component].componentsInside[
            currentComponentCall
          ].passedProps.push(ctx.ID().toString())
        : (listOfComponentsInFile[index_of_component].componentsInside[
            currentComponentCall
          ].passedProps = [ctx.ID().toString()]);
    } else {
      ...
      if (
        possibleRepeatedComponent.count >= 2 &&
        !Array.isArray(
          listOfComponentsInFile[index_of_component].componentsInside[
            currentComponentCall
          ].passedProps[0]
        ...
        listOfComponentsInFile[index_of_component].componentsInside[
          currentComponentCall
        ].passedProps[possibleRepeatedComponent.count - 1] = [
          ctx.ID().toString(),
        ];
      } else {
        ...
        listOfComponentsInFile[index_of_component].componentsInside[
          currentComponentCall
        ].passedProps[possibleRepeatedComponent.count - 1].push(
          ctx.ID().toString()
        );
      }
    }
    ...
    for (var j = 0; j < componentsCalled.length; j++) {
      ...
      if (arrayOfComponents[i].componentsInside[nameOfComponent].passedProps) {
        for (
          var k = 0;
          k <
          arrayOfComponents[i].componentsInside[nameOfComponent].passedProps
            .length;
          k++
        ) {
          componentToMatrix.props.push(
            arrayOfComponents[i].componentsInside[nameOfComponent].passedProps[
              k
            ]
          );
